#!/usr/bin/env python3
"""
overlay.py -- curated RM0440 data layered on top of the SVD skeleton.
...
fidelity; svd2od.py and build.py never need to change.
"""
import json
import os
import re
...
# --- section map (register name -> RM subsection), built from the RM TOC --------
_SECTIONS = json.load(open(os.path.join(os.path.dirname(__file__), "rm_sections.json")))


def section_for(spec, bare):
    """Resolve the RM subsection for a register.

    Order: an explicit `spec['section_map'][bare]` (needed where the generic
    'TIMx_' TOC key collides across timer chapters), then `spec['section_keyfn']`
    (turns 'MODER' -> 'GPIOx_MODER'), then the bare name in the TOC map.
    """
    explicit = spec.get("section_map", {}).get(bare)
    if explicit:
        return explicit
    keyfn = spec.get("section_keyfn")
    for key in ([keyfn(bare)] if keyfn else []) + [bare]:
        if key in _SECTIONS:
            return _SECTIONS[key]["section"]
    return None
...
GPIO = {
    ...
    "emit": [
        ...
        {"name": "GPIOC", "base_address": "0x48000800",
         "description": "General-purpose I/O port C (layout identical for GPIOD..GPIOG)",
         "note": "Identical layout/reset for GPIOD (0x48000C00), GPIOE (0x48001000), "
                 "GPIOF (0x48001400), GPIOG (0x48001800)."},
    ],
}
...
TIM_ADV = {  # Advanced-control timers, chapter 28
    ...
    "emit": [{"name": "TIM1", "note": "Advanced-control timers TIM8 (0x40013400) and "
                                      "TIM20 (0x40015000) share this register layout (RM0440 ch.28)."}],
}
TIM_GP = {  # General-purpose timers TIM2/3/4/5, chapter 29
    ...
    "fields": {"CR1": _TIM_CR1_FIELDS},
    "registers": _TIM_SR_W0C,
    "emit": [{"name": "TIM2", "note": "TIM3 (0x40000400), TIM4 (0x40000800) and "
                                      "TIM5 (0x40000C00) share this register layout (RM0440 ch.29)."}],
}
TIM_BASIC = {  # Basic timers TIM6/7, chapter 31
    ...
    "registers": _TIM_SR_W0C,
    "emit": [{"name": "TIM6", "note": "TIM7 (0x40001400) shares this register layout (RM0440 ch.31)."}],
}
